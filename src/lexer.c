...
static TokenType get_keyword_type(const char* start, size_t length) {
  ...
  if (length == 8 && str_ncmp(start, "continue", 8) == 0) {
  ...
  switch (c) {
    ...
    case '&':
      if (*lexer->current == '&') {
        ...
        if (lexer->errors) {
          SourceLocation location = {.filename = lexer->filename,
                                     .line = lexer->line,
                                     .column = lexer->column - 1,
                                     .start_pos = lexer->current - lexer->source - 1,
                                     .end_pos = lexer->current - lexer->source};

          error_list_add(lexer->errors, NULL, ERROR_LEX_INVALID_CHARACTER, ERROR_LEXICAL, location,
